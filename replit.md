...
## Recent Changes
*   **2025-10-24**: **SALES DELIVERY SAP INTEGRATION FIX** - Fixed Sales Order loading issue in SalesOrder Against Delivery module. Modified `get_so_series()` and `get_so_doc_entry()` to use standard SAP B1 OData endpoints instead of custom SQL queries. Implemented two-tier fallback for series retrieval: (1) Try SAP B1 Service Layer v2 Series endpoint for complete series list with proper names, (2) Fall back to v1 Orders query if v2 unavailable. Added comprehensive debug logging for troubleshooting SAP connection issues. Module now works out-of-the-box without requiring custom SQL query setup in SAP B1.
