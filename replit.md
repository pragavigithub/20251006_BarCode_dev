...
## Recent Changes
*   **2025-10-24**: **SALES DELIVERY SAP INTEGRATION FIX** - Fixed Sales Order loading issue in SalesOrder Against Delivery module. Modified `get_so_series()` and `get_so_doc_entry()` to use standard SAP B1 OData endpoints instead of custom SQL queries. Added comprehensive debug logging for troubleshooting SAP connection issues. Module now works out-of-the-box without requiring custom SQL query setup in SAP B1.
